...
(defun diff (num)
  "Return the function that finds the difference from the num."
  #'(lambda (x) (abs (- x num))))
...
(defparameter *cities*
  '((Atlanta 84.23 33.45) (Los-Angeles 118.15 34.03)
    ...
    (Houston 105.00 340.00) (San-Francisco 122.25 37.47)
...
(show-city-path (trip (city 'boston) (city 'san-francisco)))
...
(defun a*-search (paths goal-p successors cost-fn cost-left-fn
  ...
  "Fine a path whose state satisfies goal-p. Start with paths.
...
(defun search-gps (start goal &optional (beam-width 101))
