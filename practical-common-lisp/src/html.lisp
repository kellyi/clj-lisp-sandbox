...
(defun parse-implicit-attributes-sexp (sexp)
  (loop with tag = (first sexp)
     ...
     when (second rest)
     collect (first rest) into atttributes and
...
(defun escape-char (char)
  (case char
    ...
    (#\' "&apos;")
    (#\" "&quot;")
    (t (format nil "&s#~d;" (char-code char)))))
...
(defmethod newlint ((pp html-pretty-printer))
...
(defparameter *inline-elements*
  `(:a :abbr :acronym :address :b :bdo :big :cite :code :del :dfn :em
    :i :img :ins :kbd :label :legend :marquee :q :samp :small :span :strong :sub
    :sup :tt :var))
...
(defmethod op->code ((op (eql :toggle-indenting)) &rest operands)
  ...
  (if *pretty*
      ...
      (error "Bad op when no pretty-printing: ~a" op)))
...
(html (:ul (dolist (x '(foo bar baz)) (html (:li x)))))

(let ((x (random 10))) (html (:marquee x)))

(html (:p (:print (random 10))))
